...
def train_loop(args, labeled_dataloader, unlabeled_dataloader, meta_dataloader, test_dataloader,
    ...
    for step in range(args.start_step, args.total_steps):
        ...
        pseudo_optimizer.load_state_dict(t_optimizer.state_dict())
        # for param1, param2 in zip(teacher_model.parameters(), pseudo_model.parameters()):
        #     param2.data = param1.data.clone()
        teacher_model.train()
        # pseudo_model = copy.deepcopy(teacher_model)
        ...
        #     p_loss_reshape = torch.cat((p_loss_l_reshape, p_loss_u_reshape))
        #     p_weight = meta_model(p_loss_reshape.data)
        #     p_loss = p_weight * p_loss_reshape
        #     p_loss_l = p_loss[:batch_size].mean()
        #     p_loss_u = p_loss[batch_size:].mean()
        # else:                  # 只加权无监督样本
        #     p_loss_l = p_loss_l_vector.mean()
        #     p_weight = meta_model(p_loss_u_reshape.data)
        ...
        pseudo_grads = torch.autograd.grad(p_loss_fixmatch, pseudo_model.parameters(),
                                           create_graph=True)
        ...
        meta_optimizer.step()
        m_scheduler.step()
        del pseudo_grads,p_loss_fixmatch
...
def main():
    ...
    log_filepath = os.path.join(log_folder,
                                f"fangfa1_{args.dataset}_{args.sample_l}_{args.imba_l}_{args.sample_ul}_{args.imba_ul}_{current_datetime}.log")
    logging.basicConfig(
    ...
    if args.dataset == "cifar10":
        ...
        args.depth, args.widen_factor = 28, 8
    elif args.dataset == 'stl10':
        args.depth, args.widen_factor = 28, 2
    ...
    optim_group = t_optimizer.param_groups[0]
    pseudo_optimizer = MetaSGD(pseudo_model, pseudo_parameters,
                               lr=optim_group["lr"])
